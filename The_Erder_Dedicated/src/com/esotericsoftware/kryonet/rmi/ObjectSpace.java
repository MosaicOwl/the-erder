...
public class ObjectSpace {
	...
	protected void invoke(Connection connection, Object target,
		...
		if (!transmitReturnVal
				&& !invokeMethod.method.getReturnType().isPrimitive()) {
	...
	static private class RemoteInvocationHandler implements InvocationHandler {
		...
		public RemoteInvocationHandler(Connection connection, final int objectID) {
			...
			responseListener = new Listener() {
				public void received(Connection connection, Object object) {
					...
					responseTable.put(invokeMethodResult.responseID,
							invokeMethodResult);
        ...
        public Object invoke(Object proxy, Method method, Object[] args)
			...
			boolean needsResponse = transmitReturnValue || transmitExceptions
					|| !nonBlocking;
			if (needsResponse) {
				byte responseID;
				synchronized (this) {
	...
	static public class InvokeMethod implements FrameworkMessage,
			KryoSerializable {
		public int objectID;
        ...
        public void read(Kryo kryo, Input input) {
			...
			for (int i = 0, n = args.length; i < n; i++) {
				Serializer serializer = cachedMethod.serializers[i];
				if (serializer != null)
					args[i] = kryo.readObjectOrNull(input,
							method.getParameterTypes()[i], serializer);
				else
    ...
    static CachedMethod[] getMethods(Kryo kryo, Class type) {
		...
		PriorityQueue<Method> methods = new PriorityQueue(Math.max(1,
				allMethods.size()), new Comparator<Method>() {
			public int compare(Method o1, Method o2) {
				...
				for (int i = 0; i < argTypes1.length; i++) {
					diff = argTypes1[i].getName().compareTo(
							argTypes2[i].getName());
					if (diff != 0)
						return diff;
				}
